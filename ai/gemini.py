...
def generate(prompt: str, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    base_url = str(settings.get("gemini_base_url", "")).strip()

    if not key:
        return {
            "status": "error",
            "error": "Missing GEMINI_API_KEY (KKU IntelSphere API key)",
            "model": model,
        }
    if not model:
        return {
            "status": "error",
            "error": "Missing config/settings.json gemini_model",
            "model": model,
        }
    if not base_url:
        return {
            "status": "error",
            "error": "Missing config/settings.json gemini_base_url",
            "model": model,
        }
    ...
    for attempt in range(retries + 1):
        ...
        if not text:
            return {
                "status": "error",
                "error": "KKU Gemini response contained no text",
                "model": model,
                "raw": data,
            }
